...
class AirLLMBaseModel(GenerationMixin):
    ...
    def __init__(
        ...
        self.batch_size_layer = batch_size_layer
    ...
    def move_layer_to_device(self, state_dict):
        ...
        for param_name, param in state_dict.items():
            if self.hf_quantizer is None:
                ...
                # if hqq layer state dict has different names, so check for other names too
                if isinstance(self.hf_quantizer, HqqHfQuantizer):
                    if ".W_q" in param_name:
                        layer_name = param_name[
                            : param_name.index(".W_q") + len(".W_q")
                        ]
    ...
    def forward(
        ...
        batch = [
            input_ids_unit.to(self.running_device).unsqueeze(0).to()
        ...
        batches_layers = [
            (
                self.layer_names[i : i + self.batch_size_layer],
                self.layers[i : i + self.batch_size_layer],
            )
        ...
        with torch.inference_mode(), ThreadPoolExecutor() as executor:
            ...
            for batch_index, (batch_layer_names, batch_layers) in tqdm(
                enumerate(batches_layers),
                ...
                if self.prefetching:
                    for i, (layer_name, layer) in enumerate(
                        zip(batch_layer_names, batch_layers)
                    ):
                ...
                # if prefetching is false
                    ...
                    if self.profiling_mode:
                        t = time.time()
                    for layer_name, layer in zip(batch_layer_names, batch_layers):
                ...
                # Calculate embeddings only when embedding layer has been loaded
                if (
                    batch_index == 0
                    and batch_layer_names[0] == self.layer_names_dict["embed"]
                ):
                    inputs_embeds = batch_layers[0](input_ids)
                    position_embeddings = self.model.model.rotary_emb(
                        inputs_embeds, old_position_ids
                    )
                ...
                for moved_layers in all_moved_layers:
                    for (
                        param_name
